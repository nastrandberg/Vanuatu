...
if(0){# chiros####
  chiros<-read_excel("Emaotul_all_data.xlsx", sheet="chiro")
  zones2<-read_excel("Emaotul_all_data.xlsx", sheet="zones2")
  
  all<-merge(magsus, Ti, by="depth", all = TRUE)
  #all<-merge(all, micro, by="depth", all = TRUE)
  #all<-merge(all, macro, by="depth", all = TRUE)
  all<-merge(all, interpolated_precip, by="depth", all = TRUE)
  all<-merge(all, chiros, by="depth")
  all<-merge(all, zones2, by="depth")
  ...
  cca <- cca(chiros, env)
  cca
  plot(cca, scaling = scl)
  
  scl <- 2
  plot(cca, type = "n", scaling = scl)
  
  with(env, points(cca, display = c("sites"), col = zones,
                   scaling = scl, pch = 21, bg= zones, cex=2))
}
